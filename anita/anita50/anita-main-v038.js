(function(){
"use strict";
/* ANITA 0.3.8 — EXPLICIT BRIEF REVIEW */
if(window.__ANITA50_BOOTSTRAPPED_038__)return;
window.__ANITA50_BOOTSTRAPPED_038__=true;
const BASE="https://cdn.jsdelivr.net/gh/nodzone-cloud/anita@b0219cca86c3db272a7baa2bd5c83eb8bca83e69/anita/anita50/";
const VERSION="038";
const files=[
  "config-v017a.js",
  "state-v017.js",
...
async function boot(){try{await waitForBody();loadCSS(BASE+"anita50.css?v="+VERSION);mount();if(!document.getElementById("an50-root"))throw new Error("ANITA mount failed: #an50-root was not created");for(const file of files)await loadScript(BASE+file+"?v="+VERSION);window.__ANITA_READY_038__={singleRouter:!!window.ANITA50?.router038,core:!!window.ANITA50?.__core034,explicitBriefReview:!!window.ANITA50?.router038?.reviewBrief,websiteRequestIntent:!!window.ANITA50?.router038?.websiteRequest,explicitBriefHandoff:!!window.__ANITA_V037__?.explicitHandoffChoice,semantic:!!window.ANITA50?.semantic,ai:!!window.ANITA50?.ai,state:!!window.ANITA50?.state};console.log("[ANITA 0.3.8 EXPLICIT BRIEF REVIEW]",window.__ANITA_READY_038__);}catch(err){console.error("[ANITA 0.3.8 bootstrap]",err);window.__ANITA_BOOT_ERROR_038__=String(err&&err.message||err);}}
boot();
})();