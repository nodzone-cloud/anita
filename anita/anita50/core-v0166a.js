...
function boot(){
  ...
  if(!send || !input){
    ...
    if(bootAttempts<=120){
      ...
      console.error("[ANITA 0.1.6.6a] UI controls not ready after wait",{
        send:!!send,input:!!input
      });
      window.__ANITA_BOOT_ERROR_0166aA__="UI_CONTROLS_NOT_READY";
    }
    return;
  }

  if(send.dataset.anita166aBound==="1") return;
  send.dataset.anita166aBound="1";
