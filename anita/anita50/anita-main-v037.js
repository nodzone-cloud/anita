(function(){
"use strict";
/* ANITA 0.3.7 — EXPLICIT BRIEF HANDOFF */
if(window.__ANITA50_BOOTSTRAPPED_037__)return;
window.__ANITA50_BOOTSTRAPPED_037__=true;
const BASE="https://cdn.jsdelivr.net/gh/nodzone-cloud/anita@23b6aa38543fb1b33e5124b49c0e71ebf16cfa7c/anita/anita50/";
const VERSION="037";
const files=[
  "config-v017a.js",
  "state-v017.js",
...
async function boot(){try{await waitForBody();loadCSS(BASE+"anita50.css?v="+VERSION);mount();if(!document.getElementById("an50-root"))throw new Error("ANITA mount failed: #an50-root was not created");for(const file of files)await loadScript(BASE+file+"?v="+VERSION);window.__ANITA_READY_037__={websiteRequestIntent:!!window.ANITA50?.router034?.websiteRequest,singleRouter:!!window.ANITA50?.router034,core:!!window.ANITA50?.__core034,explicitBriefHandoff:!!window.__ANITA_V037__?.explicitHandoffChoice,emailDraftHandoff:!!window.__ANITA_V037__?.emailDraftHandoff,semantic:!!window.ANITA50?.semantic,ai:!!window.ANITA50?.ai,state:!!window.ANITA50?.state};console.log("[ANITA 0.3.7 EXPLICIT BRIEF HANDOFF]",window.__ANITA_READY_037__);}catch(err){console.error("[ANITA 0.3.7 bootstrap]",err);window.__ANITA_BOOT_ERROR_037__=String(err&&err.message||err);}}
boot();
})();