(function(){
"use strict";
/* ANITA 0.3.5 — WEBSITE REQUEST INTENT */
if(window.__ANITA50_BOOTSTRAPPED_035__)return;
window.__ANITA50_BOOTSTRAPPED_035__=true;
const BASE="https://cdn.jsdelivr.net/gh/nodzone-cloud/anita@cc961b32d9e5d5ca16f8dff1996fe1b7c9b48480/anita/anita50/";
const VERSION="035";
const files=[
  "config-v017a.js",
  "state-v017.js",
...
async function boot(){try{await waitForBody();loadCSS(BASE+"anita50.css?v="+VERSION);mount();if(!document.getElementById("an50-root"))throw new Error("ANITA mount failed: #an50-root was not created");for(const file of files)await loadScript(BASE+file+"?v="+VERSION);window.__ANITA_READY_035__={websiteRequestIntent:!!window.ANITA50?.router034?.websiteRequest,singleRouter:!!window.ANITA50?.router034,core:!!window.ANITA50?.__core034,semantic:!!window.ANITA50?.semantic,ai:!!window.ANITA50?.ai,state:!!window.ANITA50?.state};console.log("[ANITA 0.3.5 WEBSITE REQUEST INTENT]",window.__ANITA_READY_035__);}catch(err){console.error("[ANITA 0.3.5 bootstrap]",err);window.__ANITA_BOOT_ERROR_035__=String(err&&err.message||err);}}
boot();
})();