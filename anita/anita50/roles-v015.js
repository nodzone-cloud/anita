...
function cleanStoredValue(s){
  return (s||"").trim().replace(/[\s]*[.;,:!?]+$/g,"").trim();
}

function cleanSize(s){
  let v=cleanStoredValue(s);
  if(/^(A|An|One|Few|Several)\b/.test(v))v=v.charAt(0).toLowerCase()+v.slice(1);
  return v;
}

function cleanGoal(s){
  let v=cleanStoredValue(s);
  v=v.replace(/^i\s+(?:want|wanted)\s+visitors\s+to\s+/i,"");
  v=v.replace(/^visitors\s+(?:should|need to|can)\s+/i,"");
  v=v.replace(/^я\s+хочу,?\s+чтобы\s+посетители\s+/i,"");
  v=v.replace(/^хочу,?\s+чтобы\s+посетители\s+/i,"");
  return v.trim();
}

function projectSummaryForAlex(ctx,l){
  const b=ctx.websiteBrief||{};
  const m=ctx.clientMemory||{};
  const business=cleanStoredValue(b.business||m.business||"");
  const goal=cleanGoal(b.goal||"");
  const size=cleanSize(b.size||"");
  const p=b.recommendedPackage||"";
  const price=p&&C.packages[p]?C.packages[p].price:"";
  const structure=cleanStoredValue(b.recommendedStructure||"");

  if(l==="ru"){
    const parts=["Краткий бриф проекта для Алекса 😊"];
    if(business)parts.push(`Бизнес: ${business}.`);
    if(goal)parts.push(`Цель сайта: ${goal}.`);
    if(size)parts.push(`Размер: ${size}.`);
    if(structure)parts.push(`Предлагаемая структура: ${structure}.`);
    if(p)parts.push(`Рекомендованный пакет: ${p}${price?` — ${price}`:""}.`);
    parts.push("Основной бриф готов для просмотра Алексом.");
    return parts.join(" ");
  }

  const parts=["Project summary for Alex 😊"];
  if(business)parts.push(`Business: ${business}.`);
  if(goal)parts.push(`Website goal: ${goal}.`);
  if(size)parts.push(`Website size: ${size}.`);
  if(structure)parts.push(`Suggested structure: ${structure}.`);
  if(p)parts.push(`Recommended package: ${p}${price?` — ${price}`:""}.`);
  parts.push("The main brief is ready for Alex to review.");
  return parts.join(" ");
}

function affirmative(x){
  return /^(yes|yes please|please do|do it|sure|okay|ok|go ahead|да|да пожалуйста|пожалуйста|сделай|хорошо|ок)$/.test(x);
}

function wantsProjectSummary(x){
  return /(?:prepare|make|create|show|give me|send me).*?(?:project )?(?:summary|brief).*?(?:alex)?|(?:project )?(?:summary|brief).*?(?:for alex)|подготов.*(?:резюме|бриф).*алекс|сделай.*(?:резюме|бриф)|покажи.*(?:резюме|бриф)/.test(x);
}

function handlePendingAction(text,l,ctx,memory){
  const x=n(text);
  const pa=ctx.pendingAction;

  if((pa==="prepare_project_summary"&&affirmative(x))||wantsProjectSummary(x)){
    return{
      kind:"answer",role:"secretary",language:l,memory,pose:"professional",
      pendingAction:null,
      text:projectSummaryForAlex(ctx,l)
    };
  }

  if(pa==="prepare_project_summary"&&/^(no|not now|later|нет|не сейчас|позже)$/.test(x)){
    return{
      kind:"answer",role:"secretary",language:l,memory,pose:"neutral",
      pendingAction:null,
      text:say(l,"No problem 😊 I’ll keep the project brief here so we can continue later.","Хорошо 😊 Я сохраню бриф проекта здесь, и мы сможем продолжить позже.")
    };
  }
...
function briefSummary(ctx,l){
  ...
  if(b.goal){ const g=cleanGoal(b.goal); if(g) parts.push(l==="ru"?`цель сайта: ${g}`:`website goal: ${g}`); }
  if(b.size){ const z=cleanSize(b.size); if(z) parts.push(l==="ru"?`размер: ${z}`:`size: ${z}`); }
  ...
  if(b.recommendedStructure){ const st=cleanStoredValue(b.recommendedStructure); if(st) parts.push(l==="ru"?`структура: ${st}`:`structure: ${st}`); }
...
function recallBrief(text,l,ctx,memory){
  ...
  if(/what did i want visitors to do|what should visitors do|what was my website goal|what was the goal of my website|do you remember my website goal|что я хотел.*посетител|какая была цель.*сайт|ты помнишь.*цель.*сайт/.test(x)){
    if(b.goal) return{
      kind:"answer",role:"secretary",language:l,memory,pose:"professional",
      text:say(l,`You wanted visitors to ${cleanGoal(b.goal)}. 😊`,`Вы хотели, чтобы посетители ${cleanGoal(b.goal)}. 😊`)
    };
    return{
      kind:"answer",role:"secretary",language:l,memory,pose:"professional",
  ...
  if(/what size website did i want|how big.*website|how many pages.*(?:did i want|we discuss|did we discuss)|what website size.*(?:did i|we)|какой размер.*сайт|сколько страниц.*(?:хотел|обсуждали)/.test(x)){
    if(b.size) return{
      kind:"answer",role:"secretary",language:l,memory,pose:"professional",
      text:say(l,`You wanted ${cleanSize(b.size)}. 😊`,`Вы хотели ${cleanStoredValue(b.size)}. 😊`)
    };
    return{
      kind:"answer",role:"secretary",language:l,memory,pose:"professional",
  ...
  if(/can we continue.*(?:website|project)|continue.*my.*(?:website|project)|let's continue.*(?:website|project)|продолжим.*(?:сайт|проект)|можем продолжить.*(?:сайт|проект)/.test(x)){
    const summary=briefSummary(ctx,l);
    if(summary){
      return{kind:"answer",role:"secretary",language:l,memory,pose:"professional",pendingAction:"prepare_project_summary",text:`${summary} ${say(l,"I can prepare a short project summary for Alex now. Would you like me to do that?","Я могу сейчас подготовить короткое резюме проекта для Алекса. Подготовить?")}`};
    }
    return{
      kind:"answer",role:"secretary",language:l,memory,pose:"professional",
...
W.roles={
  route(text){
    ...
    const pendingResult=handlePendingAction(text,l,ctx,memory);
