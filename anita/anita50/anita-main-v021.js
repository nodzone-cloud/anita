(function(){
"use strict";
/* ANITA 0.2.1 — BRIEF FOCUS + CONVERSATION/PROJECT SEPARATION */
if(window.__ANITA50_BOOTSTRAPPED_021__)return;
window.__ANITA50_BOOTSTRAPPED_021__=true;

const BASE="https://cdn.jsdelivr.net/gh/nodzone-cloud/anita@26c547f871fb93996b1fdcc4118a7ffba722e910/anita/anita50/";
const VERSION="021";
const files=[
  "config-v017a.js",
  "state-v017.js",
...
async function boot(){try{await waitForBody();loadCSS(BASE+"anita50.css?v="+VERSION);mount();if(!document.getElementById("an50-root"))throw new Error("ANITA mount failed: #an50-root was not created");for(const file of files)await loadScript(BASE+file+"?v="+VERSION);window.__ANITA_BOOT_FILES_021__=files.slice();window.__ANITA_READY_021__={contextGuard:!!window.ANITA50?.intakeContextGuard,websiteEntry:!!window.ANITA50?.__websiteEntry020,conversationBrief:!!window.ANITA50?.__conversationBrief021,semantic:!!window.ANITA50?.semantic,ai:!!window.ANITA50?.ai,state:!!window.ANITA50?.state};console.log("[ANITA 0.2.1 BRIEF FOCUS]",window.__ANITA_READY_021__);}catch(err){console.error("[ANITA 0.2.1 bootstrap]",err);window.__ANITA_BOOT_ERROR_021__=String(err&&err.message||err);}}
boot();
})();
