...
async function handle(text){
  ...
  try{
    if(W.semantic&&W.semanticAdapter&&W.semantic.shouldTry(text)){
      ...
      const normalized=W.semanticAdapter.toNormalizedMessage(sem);
      if(normalized){
        const sr=W.roles.route(normalized);
        if(sr&&sr.kind==="answer"){
          const before=W.state.context();
          W.state.patch({
            language:sr.language||before.language,
            role:sr.role||before.role,
            topic:sr.topic!==undefined?sr.topic:before.topic,
            ...
            lastSemanticIntent:sem.intent,
            lastSemanticConfidence:sem.confidence
          });
