...
const cleanSize=s=>{let v=cleanStored(s);if(/^(A|An|One|Few|Several)\b/.test(v))v=v[0].toLowerCase()+v.slice(1);return v};
const cleanGoal=s=>cleanStored(s).replace(/^i\s+(?:want|wanted)\s+visitors\s+to\s+/i,"").replace(/^visitors\s+(?:should|need to|can)\s+/i,"").replace(/^я\s+хочу,?\s+чтобы\s+посетители\s+/i,"").trim();
function cleanBusiness(s){return cleanStored(s).replace(/\bpetshop\b/ig,"pet shop").replace(/^(?:a|an)\s+/i,"").split(/\s+(?:where|that|which|so that|because|and i want|with visitors)\s+/i)[0].trim()}
...
function briefWith(ctx,p){const b=Object.assign({business:null,goal:null,size:null,requirements:[],recommendedPackage:null,recommendedStructure:null,needsAlexReview:false},ctx.websiteBrief||{},p||{});b.requirements=[...new Set(b.requirements||[])];return b}
function extractRequirements(text){
 const x=n(text),r=[]; const add=(k,re)=>{if(re.test(x))r.push(k)};
 add("gallery",/gallery|photo gallery|галере/); add("booking",/booking|book a service|appointment|reservation|запис|брон/); add("fast contact",/fast contact|quick contact|contact button|call button|whatsapp|быстр.*связ|кнопк.*связ/); add("price list",/price list|prices|pricing|прайс|цен/); add("map",/\bmap\b|location map|google maps|карт/); add("portfolio",/portfolio|works|projects|портфолио/); add("product sales",/buy (?:my )?products|sell products|shop|checkout|cart|e-?commerce|купить|прода.*товар|магазин/); add("quote request",/request a quote|quote form|estimate|заявк.*расчет|расч[её]т/);
...
function structureFor(b){const r=b.requirements||[];const a=["Home"];if(r.includes("product sales"))a.push("Products");else a.push("Services");if(r.includes("gallery")||r.includes("portfolio"))a.push("Gallery / Portfolio");if(r.includes("price list"))a.push("Prices");a.push("About / Contact");return [...new Set(a)].join(" → ")}
...
function summary(ctx,l,forAlex=false){const b=ctx.websiteBrief||{},m=ctx.clientMemory||{},parts=[];const business=cleanStored(b.business||m.business||"");
 ...
 if(forAlex)parts.push(l==="ru"?"Основной бриф готов для просмотра Алексом.":"The main brief is ready for Alex to review."); return parts.join(" ");
}
...
function handlePending(text,l,ctx,memory){const x=n(text);if(ctx.pendingAction==="prepare_project_summary"&&affirmative(x))return{kind:"answer",role:"secretary",language:l,memory,pose:"professional",pendingAction:null,text:summary(ctx,l,true)};if(ctx.pendingAction==="prepare_project_summary"&&/^(no|not now|later|нет|не сейчас|позже)$/.test(x))return{kind:"answer",role:"secretary",language:l,memory,pendingAction:null,text:say(l,"No problem 😊 I’ll keep the brief here for later.","Хорошо 😊 Я сохраню бриф здесь на потом.")};return null}
function wantsWebsite(x){return /looking for (?:a )?website|thinking about (?:a )?website|want (?:a )?website|need (?:a )?website|want (?:a )?site|need (?:a )?site|хочу сайт|нужен сайт/.test(x)}
...
function consult(text,l,ctx){
 ...
 if(ctx.topic==="website_consultation"&&ctx.pending==="business"){
  ...
  b.business=business;return{kind:"answer",role:"secretary",language:l,topic:"website_consultation",pending:"goal",brief:b,memory:memoryWith(ctx,{business,businessRaw:text}),text:say(l,`Got it 😊 ${business}. What should visitors mainly do on the website — contact you, book, view prices, buy products, or something else?`,`Поняла 😊 ${business}. Что посетители должны делать на сайте — связываться, бронировать, смотреть цены, покупать товары или что-то ещё?`)};
 }
 ...
 if(ctx.topic==="website_consultation"&&ctx.pending==="size"){
  b.size=cleanSize(text);b=recommend(b);let msg;
...
W.roles={route(text){
 const ctx=W.state.context(),x=n(text),l=lng(text);const foundName=extractName(text),foundBusiness=extractBusiness(text);let memory=memoryWith(ctx);if(foundName)memory=memoryWith(ctx,{name:foundName});if(foundBusiness)memory=Object.assign({},memory,{business:foundBusiness,businessRaw:text});
 ...
 if(/(?:prepare|make|create|show|give me).*?(?:project )?(?:summary|brief).*alex/.test(x))return{kind:"answer",role:"secretary",language:l,memory,pose:"professional",pendingAction:null,text:summary(ctx,l,true)};
