...
function boot(){
  // v0.1.6.5h root-cause fix:
  // create the ANITA UI first, then bind controls that the UI creates.
  W.ui.ready();

  const send=s(), input=q();
  if(!send||!input){
    console.error("[ANITA 0.1.6.5h] UI controls missing after W.ui.ready()",{
      root:!!W.ui.root(),
      send:!!send,
      input:!!input
    });
    window.__ANITA_BOOT_ERROR_0165H__="UI_CONTROLS_MISSING_AFTER_READY";
    return;
  }
  ...
  window.__ANITA_UI_READY_0165H__={
    root:!!W.ui.root(),
    send:!!s(),
    input:!!q()
  };
