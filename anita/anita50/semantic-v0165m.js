...
function salvageFromConversation(original,answer){
  ...
  if(/\b(fast contact|quick contact|contact button|call button|whatsapp)\b/i.test(all)) req.push("fast contact");
  if(/\b(?:services? (?:section|page)|(?:section|page) for services?)\b/i.test(all)) req.push("services");
...
W.semantic={
  shouldTry:function(text){
    ...
    return /\b(website|web site|site|webpage|online|business|company|shop|salon|service|customer|client|booking|appointment|calendar|product|price|portfolio|gallery|contact|order|build|create|design|somewhere online|arrange a time)\b/i.test(x)
  ...
  interpret:async function(text,language){
    ...
    const prompt=[
      ...
      "REQUIREMENTS rule: add an item to requirements ONLY when the visitor explicitly asks for that feature/section or clearly says visitors must be able to do it. Do NOT infer requirements from business context. Example: 'people mostly call me' is context, NOT 'fast contact'. 'see what I do' may be a goal, NOT automatically a 'services' requirement. Never add price list unless prices/price list are explicitly requested.",
