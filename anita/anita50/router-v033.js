/* ANITA 0.3.3 — SINGLE CENTRAL ROUTER + COMMERCE CLARIFICATION */
(function(W){
"use strict";
if(!W||!W.state||W.router033)return;
...
function statusReply(x){return /^(?:good|i'?m good|im good|i am good|doing good|i'?m doing good|im doing good|i am doing good|fine|i'?m fine|im fine|i am fine|pretty good|very good|all good|not bad|okay|ok|could be better|tired|i'?m tired|im tired|bad|not great|not so good|плохо|нормально|норм|хорошо|всё хорошо|все хорошо|у меня всё хорошо|у меня все хорошо|неплохо|так себе|устал|устала|hyvin|ihan hyvin|aika hyvin|kaikki hyvin|ihan ok|ok|huonosti|väsynyt)$/.test(norm(x));}
function statusMood(x){const s=norm(x);return /^(?:bad|not great|not so good|could be better|tired|i'?m tired|im tired|плохо|так себе|устал|устала|huonosti|väsynyt)$/.test(s)?"low":"good";}
function reaction(x){return /^(?:thanks|thank you|thx|cool|nice|great|awesome|perfect|excellent|wow|lol|haha|hahaha|спасибо|круто|здорово|отлично|супер|вау|хаха|kiitos|hyvä|hienoa|mahtavaa|siisti)$/.test(norm(x));}
...
function sizeAnswer(x){const s=norm(x);return /\b(?:one|single|1)\s*(?:landing\s*)?page\b|\b(?:landing page|one-page|single-page)\b|\b(?:few|several|couple|2|3|4|5)\s+(?:separate\s+)?pages?\b|\b(?:separate pages?|multiple pages?|some pages?|more than one page)\b|\b(?:larger?|big|multi-page|multipage)\s*(?:website|site)?\b/i.test(s)||/(?:одна|1)\s+страниц|лендинг|несколько\s+страниц|отдельные страницы|многостранич/i.test(s)||/(?:yksi sivu|muutama sivu|erilliset sivut|useita sivuja|monisivuinen)/i.test(s);}
...
W.router033={
  norm,lang,greeting,smalltalk,statusReply,statusMood,reaction,meta,question,broadWebsite,newBrief,continueBrief,yes,no,sizeAnswer,goalAnswer,behaviour,alexNodeRelevant,briefActive,checkoutMode,requestMode,
  classify(text){
    ...
    if(c.socialPending==="how_are_you"&&statusReply(text))return{intent:"social_status",language:l};
    if(smalltalk(text))return{intent:"social_question",language:l};
    ...
    if(pa==="commerce_mode"){
      if(checkoutMode(text))return{intent:"commerce_checkout",language:l};
      if(requestMode(text))return{intent:"commerce_request",language:l};
      return{intent:question(text)?"side_question":"commerce_mode_unclear",language:l};
    }
