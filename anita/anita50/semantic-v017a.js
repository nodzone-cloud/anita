/* ANITA 0.1.7a — semantic requirements patch: services, information, availability calendar */
(function(W){
"use strict";
...
function explicitFacts(text){
  ...
  if(/\b(book|booking|appointment|arrange a time|schedule a time|make an appointment|reservation)\b/i.test(x)) confirmed.push("booking");
  if(/\b(?:availability|available|free|booked|occupied)\b.{0,50}\b(?:calendar|days?|dates?|slots?|times?)\b|\bcalendar\b.{0,50}\b(?:available|free|booked|occupied|slots?|days?|dates?)\b/i.test(x)) confirmed.push("availability calendar");
  ...
  if(/\b(see what i do|see what we do|see my services|see our services|view my services|view our services|services?|service list)\b/i.test(x)) confirmed.push("services");
  if(/\b(?:business )?info(?:rmation)?\b|\babout the business\b|\bcompany info(?:rmation)?\b/i.test(x)) confirmed.push("information");
  ...
  interpret:async function(text,language){
    ...
    try{
      ...
      obj.confirmed_requirements=obj.confirmed_requirements.filter(v=>{
        if(["price list","contact","fast contact","services","information","availability calendar"].includes(v) && !explicitSet.has(v)) return false;
        return true;
      });
