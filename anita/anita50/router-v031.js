/* ANITA 0.3.1 — SINGLE CENTRAL ROUTER + SOCIAL TURN AWARENESS
 * Every visitor message is classified before project/brief code can consume it.
 * The router never writes project memory.
 */
(function(W){
"use strict";
if(!W||!W.state||W.router031)return;
...
function statusMood(x){const s=norm(x);if(/^(?:bad|not great|not so good|could be better|tired|i'?m tired|im tired|плохо|так себе|устал|устала|huonosti|väsynyt)$/.test(s))return"low";return"good";}
...
W.router031={
  norm,lang,greeting,smalltalk,statusReply,statusMood,reaction,meta,question,broadWebsite,newBrief,continueBrief,yes,no,sizeAnswer,goalAnswer,behaviour,alexNodeRelevant,briefActive,
  classify(text){
