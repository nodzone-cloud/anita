/* ANITA 0.1.8 — semantic-first intake for understanding client needs */
(function(){
...
function semanticFirstDuringIntake(c,text){
  if(!c||c.topic!=="website_consultation")return false;
  if(c.pendingAction)return false;
  if(!["business","goal"].includes(c.pending))return false;
  const x=String(text||"").trim().toLowerCase().replace(/[.!?,;:]+$/,"");
  if(/^(yes|no|ok|okay|sure|да|нет|ок|kyllä|ei|joo)$/.test(x))return false;
  return true;
}
...
function applySecretaryResult(sr,sem,lang,label){
  const now=W.state.context();
  W.state.patch({
    language:sr.language||lang||now.language,
    role:sr.role||"secretary",
    topic:sr.topic!==undefined?sr.topic:(now.topic||"website_consultation"),
    ...
    lastSemanticIntent:(sem&&sem.intent)||now.lastSemanticIntent||null,
    lastSemanticConfidence:(sem&&sem.confidence)||now.lastSemanticConfidence||0,
    lastRoute:label||now.lastRoute
  });
...
async function handle(text){
  ...
  // During website intake, inspect natural client answers semantically before the old
  // deterministic next-question logic consumes them.
  if(semanticFirstDuringIntake(before,text)&&W.semantic&&W.semanticAdapter){
    const lang=detectInputLanguage(text);
    W.state.patch({language:lang,lastUser:text,lastRoute:"SEMANTIC_INTAKE_START"});
    W.ui.pose("thinking");
    ...
    try{
      ...
      if(handoff&&handoff.confidence>=0.60){
        const sr=W.semanticAdapter.routeToSecretary(handoff);
        if(sr&&sr.kind==="answer"){
          applySecretaryResult(sr,sem,lang,"SEMANTIC_INTAKE_SECRETARY");
          return;
        }
      }
      // Uncertain semantic classification falls through to the stable deterministic roles.
    }catch(e){
      // Same fallback: never guess when the semantic layer fails.
    }
  }
  ...
  if(forceSemanticWebsite(text)&&W.semantic&&W.semanticAdapter){
    ...
    try{
      ...
      if(sr&&sr.kind==="answer"){
        applySecretaryResult(sr,sem,lang,"FORCED_SEMANTIC_SECRETARY");
        return;
      }
    }catch(e){
  ...
  try{
    if(W.semantic&&W.semanticAdapter&&W.semantic.shouldTry(text)){
      ...
      if(sr&&sr.kind==="answer"){
        applySecretaryResult(sr,sem,r.language,"SEMANTIC_SECRETARY");
        return;
      }
    }
...
function bindDelegatedControls(){
  ...
  document.addEventListener("click",e=>{
    const btn=e.target&&e.target.closest?e.target.closest("#an50-send"):null;
    if(!btn)return;
    e.preventDefault();
    enqueue();
  });
  ...
  document.addEventListener("keydown",e=>{
    const input=e.target&&e.target.closest?e.target.closest("#an50-input"):null;
...
function boot(){
  ...
  window.__ANITA_UI_READY_018__={
    root:!!(W.ui.root&&W.ui.root()),
    input:!!q(),
    send:!!s(),
    delegated:true,
    semanticNeeds:true
