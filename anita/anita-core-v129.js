(function(){
  ...
  function tourIsActive(){
    try{
      const s=JSON.parse(localStorage.getItem(C.tourKey)||"null");
      return !!(s&&s.active);
    }catch(_){ return false; }
  }

  function restoreTourAfterAnswer(){
    if(!tourIsActive() || !W.tour || typeof W.tour.showCurrent!=="function") return;
    setTimeout(()=>W.tour.showCurrent(),3200);
  }

  async function handle(text){
    const result=W.router.local(text);
    W.context.patch({language:result.language,lastUser:text});
    const inTour=tourIsActive();
    ...
    if(result.kind==="answer"){
      ...
      if(inTour) restoreTourAfterAnswer();
      return;
    }
    ...
    if(result.kind==="navigate"){
      ...
      W.ui.setPose(result.pose||"ready");
      W.ui.showBubble(result.text,{autoHide:false});
      setTimeout(()=>W.actions.navigate(p.url,{target:result.target,pose:result.pose||"ready",text:result.text}),2200);
      return;
    }

    if(inTour && !C.engineUrl){
      W.ui.setPose("important");
      W.ui.showBubble(result.language==="ru"
        ?"Этот вопрос требует моей расширенной помощи 😊 Сейчас продолжим экскурсию, а к вопросу можно вернуться после неё."
        :result.language==="fi"
        ?"Tämä kysymys tarvitsee laajempaa apuani 😊 Jatketaan nyt kierrosta, ja voimme palata kysymykseen sen jälkeen."
        :"That question needs my extended assistance 😊 Let's continue the tour for now, and we can come back to it afterwards.",
        {autoHide:false});
      restoreTourAfterAnswer();
      return;
    }
