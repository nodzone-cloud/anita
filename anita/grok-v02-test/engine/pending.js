...
(function (root) {
  ...
  const QUESTIONS = {
    goal: { id:"visit_goal", field:null, type:"free_text", prompt:{en:"What brings you to Alex Node today? Looking for a website, IT help, or just exploring?",ru:"Что вас интересует? Нужен сайт, помощь с компьютером, или просто смотрите?",fi:"Mikä tuo sinut Alex Nodeen tänään? Tarvitsetko verkkosivua, IT-apua vai selailetko vain?"}, allowFreeText:true },
    business: { id:"website_business", field:"websiteBrief.confirmed.business", type:"free_text", prompt:{en:"Tell me a bit about your business or project — what do you do?",ru:"Расскажите немного о вашем бизнесе или проекте — чем вы занимаетесь?",fi:"Kerro hieman liiketoiminnastasi tai projektistasi — mitä teet?"}, allowFreeText:true },
    site_goal: { id:"website_goal", field:"websiteBrief.confirmed.goal", type:"free_text", prompt:{en:"What should visitors be able to do on the site? (contact you, book a service, buy something, learn about you…)",ru:"Что посетители должны делать на сайте? (связаться, записаться, купить, узнать о вас…)",fi:"Mitä kävijöiden pitäisi voida tehdä sivustolla? (ottaa yhteyttä, varata, ostaa, tutustua…)"}, allowFreeText:true },
    size: { id:"website_size", field:"websiteBrief.confirmed.size", type:"choice_or_count", prompt:{en:"Roughly what size of site do you need?",ru:"Какого примерно размера сайт нужен?",fi:"Minkä kokoista sivustoa tarvitset?"}, options:[{value:"one landing page",labels:{en:"One page",ru:"Одна страница",fi:"Yksi sivu"}},{value:"a few separate pages",labels:{en:"A few pages",ru:"Несколько страниц",fi:"Muutama sivu"}},{value:"a larger multi-page site",labels:{en:"Larger multi-page",ru:"Большой многостраничный",fi:"Monisivuinen"}}], allowFreeText:true, clarification:{en:"One page = a single landing page (menu items can scroll to sections). A few pages = separate pages like About / Services / Contact. Larger multi-page = many sections with their own URLs.",ru:"Одна страница = лендинг (пункты меню могут прокручивать к блокам). Несколько страниц = отдельные страницы вроде О нас / Услуги / Контакты. Большой многостраничный = много разделов со своими адресами.",fi:"Yksi sivu = laskeutumissivu (valikko voi vierittää osioihin). Muutama sivu = erilliset sivut kuten Meistä / Palvelut / Yhteystiedot. Monisivuinen = useita osioita omilla osoitteillaan."} },
    requirements: { id:"website_requirements", field:"websiteBrief.confirmed.requirements", type:"free_text", prompt:{en:"Anything specific you already know you need? (booking, shop, blog, contact form, multiple languages…)\nYou can say “nothing special” or “not sure yet”.",ru:"Есть ли что-то конкретное, что уже точно нужно? (запись, магазин, блог, форма, несколько языков…)\nМожно сказать «ничего особенного» или «пока не знаю».",fi:"Onko jotain erityistä, mitä jo tiedät tarvitsevasi? (ajanvaraus, kauppa, blogi, lomake, useita kieliä…)\nVoit sanoa “ei mitään erityistä” tai “en ole varma vielä”."}, allowFreeText:true },
    confirm_brief: { id:"confirm_brief", field:null, type:"confirm_yes_no", prompt:{en:"Did I understand everything correctly?",ru:"Всё правильно поняла?",fi:"Ymmärsinkö kaiken oikein?"}, options:[{value:"yes",labels:{en:"Yes, correct ✓",ru:"Да, всё верно ✓",fi:"Kyllä, oikein ✓"}},{value:"no",labels:{en:"Not quite",ru:"Не совсем",fi:"Ei aivan"}}], allowFreeText:true }
  };
  ...
  function interpret(pending,text,language){
    if(!pending)return{kind:"no_pending"}; const x=norm(text),lang=language||"en";
    if(/(?:в чем|какая|объясни|поясни|что значит|чем).{0,40}(?:разниц|отлич)|difference|what(?:'s| is) the difference|mitä eroa|selitä/i.test(x)||/^(а )?(в чём|в чем) разница/i.test(x)) return{kind:"clarification",text:clarificationOf(pending,lang)||(lang==="ru"?"Могу объяснить варианты 😊 Напишите, что именно сравнить.":"I can explain the options 😊 Tell me what you’d like compared.")};
    ...
    if(pending.type==="confirm_yes_no"){if(/^(да|yes|верно|правильно|всё верно|ok|okay|looks good|kyllä|joo)$/i.test(x))return{kind:"answer",value:"yes"};if(/^(нет|no|не совсем|not really|ei)$/i.test(x))return{kind:"answer",value:"no"};}
    if(pending.options&&pending.options.length){
      ...
      const numMatch=x.match(/(?:вариант|option)?\s*(\d{1,2})/i); if(numMatch){const n=parseInt(numMatch[1],10);if(pending.type==="choice"&&n>=1&&n<=pending.options.length)return{kind:"answer",value:pending.options[n-1].value};if(pending.type==="choice_or_count"&&pending.id==="website_size"){if(n===1)return{kind:"answer",value:"one landing page"};if(n>=2&&n<=5)return{kind:"answer",value:"a few separate pages"};if(n>5)return{kind:"answer",value:"a larger multi-page site"};}}
  ...
  W.engine=W.engine||{};W.engine.Pending={QUESTIONS,get:(id)=>QUESTIONS[id]||null,interpret,promptOf,clarificationOf,detectLanguage,norm};
